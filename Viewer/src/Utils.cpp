...
glm::mat4x4 Utils::TranslationMatrix(const glm::vec3& translationVector)
{
...
glm::mat4x4 Utils::AxisRotationMatrix(glm::vec3 axis, double angle)
{
	double xzProjectionLength = glm::sqrt(axis.x * axis.x + axis.z * axis.z);
	double theta = atan2(axis.x, axis.z);
	double phi = atan2(axis.y, xzProjectionLength);

	glm::mat4x4 yRotation = Utils::YRotationMatrix(theta);
	glm::mat4x4 xRotation = Utils::XRotationMatrix(-phi);
	glm::mat4x4 yInverseRotation = glm::transpose(yRotation);
	glm::mat4x4 xInverseRotation = glm::transpose(xRotation);
	glm::mat4x4 zRotation = Utils::ZRotationMatrix(angle);

	return yInverseRotation * xInverseRotation * zRotation * xRotation * yRotation;
}

glm::mat4x4 Utils::ScalingMatrix(const glm::vec3& scalingVector)
{
	glm::mat4x4 scalingMatrix;
	scalingMatrix[0] = glm::vec4(scalingVector.x, 0, 0, 0);
	scalingMatrix[1] = glm::vec4(0, scalingVector.y, 0, 0);
	scalingMatrix[2] = glm::vec4(0, 0, scalingVector.z, 0);
	scalingMatrix[3] = glm::vec4(0, 0, 0, 1);
	return scalingMatrix;
}

glm::mat4x4 Utils::XScalingMatrix(double factor)
{
	glm::mat4x4 scalingMatrix;
...
glm::mat4x4 Utils::YScalingMatrix(double factor)
{
	glm::mat4x4 scalingMatrix;
...
glm::mat4x4 Utils::ZScalingMatrix(double factor)
{
	glm::mat4x4 scalingMatrix;
	scalingMatrix[0] = glm::vec4(1, 0, 0, 0);
	scalingMatrix[1] = glm::vec4(0, 1, 0, 0);
